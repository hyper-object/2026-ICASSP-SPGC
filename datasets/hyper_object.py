...
class HyperObjectDataset(HSIDataset):
    ...
    def __init__(
        ...
        mosaic_path=ModalitySpec(root=Path(f"{data_root}/{'train' if train else 'test'}/mosaic"), exts=(".png", ".jpg"))
        rgb_path=ModalitySpec(root=Path(f"{data_root}/{'train' if train else 'test'}/rgb"),    exts=(".png", ".jpg"))
        rgb_2_path=ModalitySpec(root=Path(f"{data_root}/{'train' if train else 'test'}/rgb_2"),    exts=(".png", ".jpg"))
        rgb_4_path=ModalitySpec(root=Path(f"{data_root}/{'train' if train else 'test'}/rgb_4"),    exts=(".png", ".jpg"))
    ...
    def _load_(self, stem: str):
        ...
        p_rgb = self._maps["rgb"][stem]
        p_rgb_2 = self._maps["rgb_2"][stem]
        p_rgb_4 = self._maps["rgb_4"][stem]
        p_hsi = self._maps["hsi"][stem]

        mosaic = read_mosaic(p_mosaic)       # (H,W,1) float32 [0,1]
        rgb = read_rgb_image(p_rgb)             # (H,W,3) float32 [0,1]
        rgb_2 = read_rgb_image(p_rgb_2)         # (H,W,3) float32 [0,1]
        rgb_4 = read_rgb_image(p_rgb_4)         # (H,W,3) float32 [0,1]
        cube = read_h5_cube(p_hsi, 'cube')      # (H,W,C)

        # HWC -> CHW
        mosaic_t = torch.from_numpy(np.transpose(mosaic, (2, 0, 1)))
        rgb_t = torch.from_numpy(np.transpose(rgb, (2, 0, 1)))      # 3,H,W
        rgb_2_t = torch.from_numpy(np.transpose(rgb_2, (2, 0, 1)))
        rgb_4_t = torch.from_numpy(np.transpose(rgb_4, (2, 0, 1)))
        cube_t = torch.from_numpy(np.transpose(cube, (2, 0, 1)))    # C,H,W
        return mosaic_t, rgb_t, rgb_2_t, rgb_4_t, cube_t

    def __getitem__(self, idx: int):
        stem = self.ids[idx]
        mosaic_t, rgb_t, rgb_2_t, rgb_4_t, cube_t = self._load_(stem)

        # Apply transforms
        if self.transforms is not None:
            # joint transform expects dict
            out = self.transforms({"mosaic": mosaic_t, "rgb": rgb_t, "rgb_2": rgb_2_t, "rgb_4": rgb_4_t, "cube": cube_t, "id": stem})
            mosaic_t, rgb_t, rgb_2_t, rgb_4_t, cube_t = out["mosaic"], out["rgb"], out["rgb_2"], out["rgb_4"], out["cube"]
